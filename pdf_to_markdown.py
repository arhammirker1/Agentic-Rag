...
For SCANNED PDFs (no embedded text), automatically falls back to OCR via
PyMuPDF (page rendering) + Tesseract (OCR), processed in parallel for speed.
...
Usage:
    ...
    python pdf_to_markdown.py scanned.pdf --ocr-dpi 300
"""
...
class PDFToMarkdown:
    ...
    # Tesseract executable dirs to add to PATH on Windows
    _TESSERACT_DIRS = [
        r"C:\Program Files\Tesseract-OCR",
        r"C:\Program Files (x86)\Tesseract-OCR",
    ]

    def __init__(self, pdf_path: str | Path, ocr_lang: str = "eng",
                 deskew: bool = True):
        ...
        # Will point to searchable PDF if the input was scanned
        self._active_pdf_path = self.pdf_path

        # If the PDF is scanned, bake a text layer first, then open that
        if self._pdf_is_scanned():
            self._active_pdf_path = self._make_searchable_pdf()

        self.pdf = pdfplumber.open(str(self._active_pdf_path))
    ...
    # ------------------------------------------------------------------
    # Scanned PDF detection + ocrmypdf text-layer baking
    # ------------------------------------------------------------------

    def _pdf_is_scanned(self) -> bool:
        """
        Quick pre-check: open with pdfplumber, sample the first few pages.
        If none of them have meaningful embedded text, it's a scanned PDF.
        """
        try:
            import pdfplumber
            with pdfplumber.open(str(self.pdf_path)) as pdf:
                ...
                for page in sample:
                    ...
                    if len(text) >= 20:
                        return False   # found real text on at least one page
            return True
        except Exception:
            return False

    def _tesseract_env(self) -> dict:
        """Build an env dict that ensures Tesseract is on PATH."""
        env = os.environ.copy()
        for tdir in self._TESSERACT_DIRS:
    ...
    def _make_searchable_pdf(self) -> Path:
        """
        Run ocrmypdf to embed an invisible text layer into the scanned PDF.
        Stores the result in a temp directory and returns its path.
        ocrmypdf uses Tesseract internally with better preprocessing
        (deskew, denoise) than raw pytesseract calls.
        """
        try:
            import ocrmypdf  # noqa: F401 — just check it's importable
        except ImportError:
            raise ImportError(
                "ocrmypdf required for scanned PDFs:\n"
                "  pip install ocrmypdf\n"
                "  (Tesseract must also be installed)"
            )

        self._tmp_dir = tempfile.TemporaryDirectory(prefix="pdf2md_ocr_")
        out_path = Path(self._tmp_dir.name) / self.pdf_path.name
        ...
        if self._deskew:
            cmd.append("--deskew")
        cmd += [str(self.pdf_path), str(out_path)]

        total_pages = self._count_pages()
        print(
            f"  [OCR] Scanned PDF detected ({total_pages} page(s)) — "
            f"baking text layer via ocrmypdf …",
            file=sys.stderr, flush=True
        )

        result = subprocess.run(cmd, env=self._tesseract_env(),
                                capture_output=True, text=True)

        # Exit code 6 = already has text (--skip-text kept it as-is), that's fine
        if result.returncode not in (0, 6):
            raise RuntimeError(
                f"ocrmypdf failed (exit {result.returncode}):\n{result.stderr}"
            )

        print("  [OCR] Done — searchable PDF ready", file=sys.stderr, flush=True)
        return out_path

    def _count_pages(self) -> int:
        """Return total page count without opening a full pdfplumber session."""
        try:
            import pdfplumber
            with pdfplumber.open(str(self.pdf_path)) as pdf:
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert PDF to structured Markdown. Scanned PDFs are handled '
                    'automatically via ocrmypdf (bakes a text layer, then uses '
                    'the same high-quality pdfplumber extraction as normal PDFs).'
    )
    ...
    parser.add_argument(
        '--lang', default='eng',
        help='Tesseract language for scanned PDFs, e.g. eng, jpn, eng+jpn (default: eng)'
    )
    parser.add_argument(
        '--no-deskew', action='store_true',
        help='Skip deskew step for scanned PDFs (faster, worse for tilted scans)'
    )
